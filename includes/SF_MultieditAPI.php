...
class SFMultieditAPI extends ApiBase {
	...
	protected function getFormTitle() {
		...
		if ( $this->mOptions['form'] === '' ) {
			...
			if ( count( $formNames ) === 0 ) {
				...
				if ( count( $formNames ) === 0 ) {
					...
					if ( count( $formNames ) === 0 ) {
						throw new MWException( wfMessage( 'sf_multiedit_noformfound' )->parse() );
					}
			...
			if ( count( $formNames ) > 1 ) {
				throw new MWException( wfMessage( 'sf_multiedit_toomanyformsfound' )->parse(), self::DEBUG );
			}
		...
		if ( !($formTitle instanceOf Title) ) {
			throw new MWException( wfMessage( 'sf_multiedit_invalidform', $this->mOptions['form'] )->parse() );
		}
		...
		if ( $formTitle->isRedirect() ) {
			...
			if ( $formTitle->isRedirect() ) {
				...
				if ( $newTitle instanceOf Title && $newTitle->isValidRedirectTarget() ) {
					throw new MWException( wfMessage( 'sf_multiedit_redirectlimitexeeded', $this->mOptions['form'] )->parse() );
				} else {
					throw new MWException( wfMessage( 'sf_multiedit_invalidredirecttarget', $newTitle->getFullText(), $this->mOptions['form'] )->parse() );
				}
			}
		}
	...
	protected function doStore( EditPage $editor ) {
		...
		if ( $this->getRequest()->getBool( 'redlink' ) && $title->exists() ) {
			$this->logMessage( wfMessage( 'sf_multiedit_redlinkexists' )->parse(), self::WARNING );
		}
		...
		if ( $permErrors ) {
			...
			// Multi-block user's IP if the account was "hard" blocked
...
//				return true; // fail
	...
	protected function finalizeResults() {
		...
		if ( $this->mStatus === 200 ) {
			if ( array_key_exists( 'ok text', $this->mOptions ) ) {
				...
				$responseText = wfMessage( 'sf_multiedit_success', $this->mOptions['target'], $this->mOptions['form'] )->parse();
			} else {
				$responseText = null;
			}
		} else {
			...
			if ( array_key_exists( 'error text', $this->mOptions ) ) {
				...
				$responseText = wfMessage( 'sf_multiedit_fail', $this->mOptions['target'] )->parse();
			} else {
				$responseText = null;
			}
		}
	...
	protected function generateTargetName( $targetNameFormula ) {
		...
		if ( preg_match( '/{num.*}/', $targetName, $matches ) && strpos( $targetName, '{num' ) !== false ) {
			...
			// if the specified target title is invalid, give up
			if ( !$targetTitle instanceof Title ) {
				throw new MWException( wfMessage( 'sf_multiedit_invalidtargetspecified', trim( preg_replace( '/<unique number(.*)>/', $titleNumber, $targetNameFormula ) ) )->parse() );
			}
	...
	public function doAction() {
		...
		if ( wfReadOnly() ) {
			...
			if ( $this->mAction === self::ACTION_SAVE ) {
				throw new MWException( wfMessage( 'sf_multiedit_readonly', wfReadOnlyReason() )->parse() );
			}
	...
	function getDescription() {
		return <<<END
This module is used to remotely create or edit multiple pages using Semantic Forms.
